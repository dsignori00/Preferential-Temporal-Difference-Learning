...
parser = ArgumentParser(description="Parameters for the code - ptd")
...
errors = []
emp_state_error = []
progress = Progress(args.episodes, f"ptd env={args.env} seed={args.seed} lr={args.lr} lamb={args.lamb}")

for n_epi in range(args.episodes):

	curr_s, c_s = env.reset()
	...
	while(not done):
		...
		if np.all(np.isfinite(new_weights)):
			...
			print(f"ptd.py: Discarded overflowing weight update at episode {n_epi}")
		c_s = n_s
		curr_s = next_s
	...
	if args.log == 1:
		...
		if np.all(np.isfinite(sq_error)):
			...
			print(f"ptd.py: Discarded overflowing estimate at episode {n_epi}")

	progress.update(n_epi + 1)

if args.log == 1 and args.save == 1:
	filename = "ptd"+"_env_"+str(args.env)+"_lamb_"+str(args.lamb)+"_lr_"+str(args.lr)+"_seed_"+str(args.seed)
