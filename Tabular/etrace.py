...
errors = []
emp_state_error = []
progress = Progress(args.episodes, f"etrace env={args.env} seed={args.seed} lr={args.lr} lamb={args.lamb}")
...
for n_epi in range(args.episodes):
	...
	while(not done):
		...
		if np.all(np.isfinite(new_weights)):
			...
			print(f"etrace.py: Discarded overflowing weight update at episode {n_epi}")
		c_s = n_s
		curr_s = next_s
	...
	if args.log == 1:
		...
		if np.all(np.isfinite(sq_error)):
			...
			print(f"etrace.py: Discarded overflowing estimate at episode {n_epi}")
