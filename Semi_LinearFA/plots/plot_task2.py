...
length = [8, 12, 16]
env = "gridWorld2"
epi = {8:150, 12:100, 16:100}
z_star = 0.674 # 0.674-50% 1.28-80%, 1.645-90%, 1.96-95%, 2.33-98%, 2.58-99%
pdf = matplotlib.backends.backend_pdf.PdfPages("plots/Semi_Grid_task2_test.pdf")
...
learn_rate = {"etd_int_0.01":{8:0.003, 12:0.001, 16:0.001} , "etd_int_0.05":{8:0.001, 12:0.0003, 16:0.0003} , "etd_int_0.1":{8:0.0003, 12:0.0001, 16:0.0001} ,\
        "accumulating":{8:0.001, 12:0.0003, 16:0.0003} , "ptd":{8:0.003, 12:0.001, 16:0.0007},\
...
for c_l in length:
    for t in types:
        ...
        for se in seed:
            try:
                with open("results_gridWorld2/drl_"+t+"_env_"+env+"_size_"+str(c_l)+"_lr_"+str(learn_rate[t][c_l])+"_seed_"+str(se)+"_epi_"+str(epi[c_l])+"_all_errors.pkl", "rb") as f:
        ...
        ax[row_plt, col_plt].set_xlabel("Episodes", fontsize=15)
        ax[row_plt, col_plt].set_ylabel("MSE on FO states", fontsize=15)
