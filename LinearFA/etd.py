...
errors = []
emp_state_error = []
progress = Progress(args.episodes, f"etd env={args.env} seed={args.seed} lr={args.lr} len={args.len}")
...
for n_epi in range(args.episodes):
	...
	while(not done):
		...
		if np.all(np.isfinite(new_weights)):
			...
			print(f"etd.py: Discarded overflowing weight update at episode {n_epi}")
