...
parser.add_argument('--intrst', type=float, default=0.95, help="interest value")
...
if args.env == "YChain":
	filename = "etd_adaptive_4"+"_env_"+str(args.env)+"_len_"+str(args.len)+"_lr_"+str(args.lr)+"_seed_"+str(args.seed)
...
errors = []
emp_state_error = []
progress = Progress(args.episodes, f"etd_2 env={args.env} seed={args.seed} lr={args.lr} len={args.len}")
...
for n_epi in range(args.episodes):
	...
	while(not done):
		...
		if np.all(np.isfinite(new_weights)):
			...
			print(f"etd_2.py: Discarded overflowing weight update at episode {n_epi}")
	...
	if args.log == 1:
		...
		if np.all(np.isfinite(sq_error)):
			...
			print(f"etd_2.py: Discarded overflowing estimate at episode {n_epi}")
