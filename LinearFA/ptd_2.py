...
if args.env == "YChain":
	filename = "ptd_4"+"_env_"+str(args.env)+"_len_"+str(args.len)+"_lr_"+str(args.lr)+"_seed_"+str(args.seed)
...
errors = []
emp_state_error = []
progress = Progress(args.episodes, f"ptd_2 env={args.env} seed={args.seed} lr={args.lr} len={args.len}")
...
for n_epi in range(args.episodes):
	...
	if args.log == 1:
		...
		if np.all(np.isfinite(sq_error)):
			...
			print(f"ptd_2.py: Discarded overflowing estimate at episode {n_epi}")
